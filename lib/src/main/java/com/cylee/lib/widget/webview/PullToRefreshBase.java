...
public class PullToRefreshBase extends LinearLayout {
    ...
    private boolean isJustEnable 		= true; // 此时控件是否有效，为短暂性
    ...
    public void setJustEnableGesture(boolean enable) {
    	isJustEnable = enable;
    }
	...
	public void setRefreshableView(WebView refreshableView){
	...
	protected boolean isReadyForPullDown() {
		if(refreshableView == null) return this.getScrollY() == 0;
	...
	protected boolean isReadyForPullUp() {
		...
		if(refreshableView instanceof WebView){
			return refreshableView.getScrollY() >= (((WebView)refreshableView).getContentHeight() - refreshableView.getHeight());
		}else{
			return refreshableView.getScrollY() >=0;
		}
	}
...
//			this.setBackgroundResource(a.getResourceId(R.styleable.PullToRefresh_adapterViewBackground,
//					Color.WHITE));
//		}
//		a.recycle();
...
//				newHeight = Math.round((initialMotionY - lastMotionY) / FRICTION);
		...
		switch (currentMode) {
		case MODE_PULL_UP_TO_REFRESH:
			footerLayout.setPullPreLabel(Math.abs(newHeight*100/headerHeight));
			break;
		case MODE_PULL_DOWN_TO_REFRESH:
			headerLayout.setPullPreLabel(Math.abs(newHeight*100/headerHeight));
			break;
		}
		if (newHeight != 0) {
